...
var _host = 'http://192.168.191.1/';
function _ajax (type, url, data, callback) {
	var query = function (type, url, data, callback) {
		mui.ajax(url, {
			type: type,
			data: data,
			dataType: 'json',
			timeout: 10000,
			headers: { ssid: localStorage.ssid },
			success: callback,
			error: function(xhr, desc, errorThrown){
				if(errorThrown == "Forbidden") {
					localStorage.time_out = 0;
					_ajax(type, url, data, callback);
				}
			}
		});
	}
	if(!localStorage.ssid || localStorage.time_out < Date.now() / 1000 + 60) {
		mui.ajax(_host + 'api/ssid', {
			type: 'get',
			dataType:'json',
			timeout: 10000,
			headers: { ssid: localStorage.ssid },
			success: function (res) {
				localStorage.ssid = res.ssid;
				localStorage.time_out = res.time_out;
				query(type, url, data, callback);
			},
			error: function(xhr, desc, errorThrown){
				mui.toast('网络连接出错...');
			}
